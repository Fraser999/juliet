...
use crate::{
    protocol::{payload_is_multi_frame, LocalProtocolViolation, MaxFrameSize},
    ChannelId, Id,
};
...
impl WaitQueue {
    ...
    pub(super) fn try_push_back(
        &mut self,
        item: QueuedItem,
        request_allowed: bool,
        multi_frame_allowed: bool,
        ...
        match item {
            QueuedItem::Request {
                ...
                if !request_allowed {
                    ...
                    self.push_request(channel, io_id, payload, permit, max_frame_size);
                    return Ok(PushOutcome::Pushed);
                }

                if self.has_active_multi_frame(&payload, max_frame_size, multi_frame_allowed) {
                    #[cfg(feature = "tracing")]
    ...
    fn has_active_multi_frame(
        &self,
        payload: &Option<Bytes>,
        max_frame_size: MaxFrameSize,
        multi_frame_allowed: bool,
    ) -> bool {
        if multi_frame_allowed {
            return false;
        };

        if let Some(payload) = payload {
            return payload_is_multi_frame(max_frame_size, payload.len());
        }
