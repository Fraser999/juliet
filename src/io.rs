    ...
    W: AsyncWrite + Unpin,
{
    ...
    fn handle_incoming_item(&mut self, item: QueuedItem) -> Result<(), LocalProtocolViolation> {
        ...
        // Add the item to the wait queue, or the ready queue if the wait queue returns the item.
        let request_allowed = self.juliet.allowed_to_send_request(channel)?;
        let multi_frame_allowed = self.active_multi_frame[channel.get() as usize].is_none();
        match self.wait_queue[channel.get() as usize].try_push_back(
            item,
            request_allowed,
            multi_frame_allowed,
            self.juliet.max_frame_size(),
        )? {
    ...
    fn process_wait_queue(&mut self, channel: ChannelId) -> Result<(), LocalProtocolViolation> {
        loop {
            ...
            match wait_queue.next_item(request_allowed, multi_frame_allowed)? {
                Some(item) => self.send_to_ready_queue(item)?,
                None => return Ok(()),
            }
        }
    }
